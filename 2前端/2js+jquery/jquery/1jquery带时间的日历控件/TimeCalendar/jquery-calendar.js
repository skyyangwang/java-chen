...
var popUpCal = {
	...
	monthNames: ['01','02','03','04','05','06','07','08','09','10','11','12'], // Names of months
	...
	/*fomate string ninggy*/	
	formatString: function() {
		var currentDate = this.parseDate(this.input.val());
		return currentDate.getFullYear()+'-'+(currentDate.getMonth()<10?'0'+currentDate.getMonth():currentDate.getMonth())+'-'+(currentDate.getDate()<10?'0'+currentDate.getDate():currentDate.getDate())+' '+(currentDate.getHours()<10?'0'+currentDate.getHours():currentDate.getHours())+':'+(currentDate.getMinutes()<10?'0'+currentDate.getMinutes():currentDate.getMinutes())+':00';
	},
	...
	hourString: function(Hour) {
		...
		else return  Hour<10?'0'+Hour:Hour;
	},
	...
	showCalendar: function() {
		...
		var timeSelect = '';
		if (this.timeSeparators && this.timeSeparators.length > 1) {
			timeSelect += '<div id="calendar_time">时间：<select id="calendar_hour">';
			...
			timeSelect += '</select>小时 ' + this.timeSeparators[1] + ' <select id="calendar_minute">';
			for (var a = 1; a < 60; a++) {
		...
		var html ='<div id="calendar_control">'

		var daySelect = '';
		//alert(timeSelect);
		if (!this.changeMonth) {
			daySelect += this.monthNames[this.selectedMonth] + '&nbsp;';
		}
		else {
		...
		if (!this.changeYear) {
			...
			daySelect += '<select id="calendar_newYear">';
			for (; year <= endYear; year++) {
				daySelect += '<option value="' + year + '"' + 
					(year == this.selectedYear ? ' selected="selected"' : '') + 
					'>' + year + '</option>';
			}
			daySelect += '</select>';
		}
		html += '<div id="calendar_links"><a  href="javascript:void(0)" id="calendar_prev" title="&lt;上月">' + this.prevText + '</a>' +
			'<span id="calendar_current1">' + daySelect + '</span>' +
			'<a href="javascript:void(0)" id="calendar_next" title="下月&gt;">' + this.nextText + '</a></div>' 
+timeSelect + '<table id="calendar" cellpadding="0" cellspacing="0"><thead>' +
